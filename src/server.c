...
void respond_request(http_request_t *req) {
    ...
    if (app_config.audio_enable && EQUALS(req->uri, "/audio.mp3")) {
        respLen = sprintf(response,
            ...
            "Transfer-Encoding: chunked\r\n"
            "Connection: keep-alive\r\n\r\n");
        register_stream_client(req->clntFd, STREAM_MP3, response, respLen);
        return;
    }
    ...
    if (app_config.audio_enable && EQUALS(req->uri, "/audio.pcm")) {
        respLen = sprintf(response,
            ...
            "Transfer-Encoding: chunked\r\n"
            "Connection: keep-alive\r\n\r\n");
        register_stream_client(req->clntFd, STREAM_PCM, response, respLen);
        return;
    }
    ...
    if (app_config.mjpeg_enable && EQUALS(req->uri, "/mjpeg")) {
        ...
        register_stream_client(req->clntFd, STREAM_MJPEG, response, respLen);
        return;
    }
    ...
    if (EQUALS(req->uri, "/api/isp")) {
        ...
        respLen = sprintf(response,
            ...
            "Connection: close\r\n"
            "\r\n"
            "{\"mirror\":%s, \"flip\":%s, \"antiflicker\":%d, \"level3dnr\":%d}",
            app_config.mirror ? "true" : "false", app_config.flip ? "true" : "false",
            app_config.antiflicker, app_config.level3dnr);
        send_and_close(req->clntFd, response, respLen);
        return;
    }
